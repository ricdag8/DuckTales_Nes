...
struct Vec2D {
    ...
    // casting operators
    operator Vec2D < int32_t > () const {
        return {
            static_cast < int32_t > (this -> x),
            static_cast < int32_t > (this -> y)
        };
    }
    operator Vec2D < float > () const {
        return {
...
template < class T > inline Vec2D < T > operator * (const float & lhs,
                          const Vec2D < T > & rhs) {
    return Vec2D < T > ((T)(lhs * (float) rhs.x), (T)(lhs * (float) rhs.y));
}
template < class T > inline Vec2D < T > operator * (const double & lhs,
                          const Vec2D < T > & rhs) {
    return Vec2D < T > ((T)(lhs * (double) rhs.x), (T)(lhs * (double) rhs.y));
}
template < class T > inline Vec2D < T > operator * (const int & lhs,
                          const Vec2D < T > & rhs) {
